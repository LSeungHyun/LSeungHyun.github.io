...
### 4. [NineTailFox Adventure 구성요소](#4)
### 5. [게임 시스템 디자인](#5)
### 6. [개발 요구사항 & 흐름도](#6)
#### 6-1. [개발 요구사항 & 흐름도(1년차)](#61)
#### 6-2. [개발 요구사항 & 흐름도(6주차)](#62)
### 7. [시간별 흐름도 flowchart](#7)
### 8. [키보드 이벤트에 대한 흐름도 flowchart](#8)
### 9. [용어 정리](#9)
### 10. [스토리보드](#10)
### 11. [Github Blog URL](#11)
### 12. [6주 요구사항 작업결과](#12)
...
> ### 대표이미지 기반 : 주인공 미호가 동료를 구출해 나가며 미로들을 탈출한다.

> ### 컨셉 기반: 탈출,구출의 컨셉을 기반으로 하여 작품을 묘사하였다.
...
|Stand(가만히 서 있는 상태)|Characteristic(특성)|R 키 입력시 동물이 특성을 사용함|
|Stand(가만히 서 있는 상태)|Die(사망)|함정에 충돌했을 때|
|Walk(정면으로 이동)|Die(사망)|함정에 충돌했을 때|
|Walk(정면으로 이동)|Characteristic(특성)|R 키 입력시 동물이 특성을 사용함|
|Die(사망)|Stand(가만히 서 있는 상태)|함정에 충돌했을 때|
...
|크랙형|crack|Snake 천장이 낮고 좁은 틈을 이동할 수 있다.|
|공중형|fly|Bird 점프로 올라가지 못하는 높은 지대를 올라갈 수 있다.|
|스피드형|speed|Deer 빠르게 이동해야 하는 구간을 통과할때 용이하다.|
...
1) 핵심 규칙
유저는 기본적으로 Miho라는 여우 캐릭터로 구성됨.
외딴섬 박사 실험실의 각종 장애물과 함정을 피해 이동하며, 다른 동 실험실에 갇혀있는 동물들을 구출.
구출한 동물들로 Miho가 변신을 할 수 있게되며 변신한 동물 캐릭터들 또한 유저의 play캐릭터로 동일함.
구출한 동물들은 각각의 동물 특성을 가지고 있으며, 박사 실험실에서 탈출 시에 나오는 각종 지형지물과 함정을 통과하는데 필요한 특성으로 구성되어있음.
지형의 특징을 잘 파악하여 지형을 통과하는데 용이한 동물의 특성을 이용해야함.
플레이하는 중간마다 SavePoint가 있으며 사망시 마지막 SavePoint로 이동하게 됨.
맵 중간마다 나오는 코인을 먹으면 재시도 할 수 있는 Life를 획득하게 됨.
모든 Life 소진시 게임을 처음부터 재시작 해야함.
...
-3.빙판길 직전 위치로 이동함
-4.펭귄이 빙판길 입장 후 특성을 발휘하게 될 시 Drag값을 0으로 설정함
...
### 5. 맵 디자인 요구사항
5-1 1스테이지 컨셉인 실험실 레벨디자인을 애셋을 활용하여 구현한다.

~~5-2 2스테이지 컨셉인 레벨디자인을 애셋을 활용하여 구현한다.~~

~~5-3 3스테이지 컨셉인 레벨디자인을 애셋을 활용하여 구현한다.~~

~~5-4 4스테이지 컨셉인 레벨디자인을 애셋을 활용하여 구현한다.~~

~~5-5 5스테이지 컨셉인 레벨디자인을 애셋을 활용하여 구현한다.~~

~~5-6 6스테이지 컨셉인 레벨디자인을 애셋을 활용하여 구현한다.~~
...
## NinetailFoxAdventure의 요구사항 및 개발작업 정도 (6주차) <a name='62'></a>
### 1주차
...
2-4 조작법을 누르면 게임 내에서 사용 가능한 키보드 입력 값과 설명팝업이 화면에 표시된다. (4-4 이승현) 50% 
<br> <br>
    (조작법 버튼 누르면 창 뜨는것은 구현 완료 / 조작법 이미지 넣으면 완성됨.)

2-5 ~~환경설정 창에서는 사운드 조절과 게임종료, 돌아가기, 조작법을 보여준다.~~ (4-5 이승현) 100%

2-6 게임종료 버튼 클릭시 '현재 진행된 부분까지 저장됩니다'라는 팝업이 표시되며 확인 버튼을 누르면 게임이 저장된다. (3-6 이승현, 임형균) 50%
<br> <br>
    (게임종료 버튼 클릭 시 '현재 진행된 부분까지 저장됩니다' 텍스트 출력 및 확인 버튼까지 구현 완료 / 확인 버튼 누르면 게임 저장될 수 있도록 해야함)

### 3주차

3-1 ~~함정에 충돌시 사망하며 Life가 1 감소된다.~~ (1-3 이승현) 100%

3-2 ~~사망 후 남은 Life가 0이라면 게임의 처음으로 되돌아가게 되며, 남은 Life의 개수가 0을 초과한다면 마지막 SavePoint로 이동하게된다.~~ (1-13 이승현) 100%

3-3 ~~플레이어가 SavePoint존에 들어가게되면 현재 SavePoint의 Position값과 구출한 동료가 저장된다.~~ (3-4 임형균) 100%
...
4-4 1스테이지 컨셉인 실험실 레벨디자인을 애셋을 활용하여 구현한다. (5-1 이승현, 임형균) 50%
    레벨 디자인 계속 이어나가고 좀더 넓힐것. 중간중간 장애물도 넣고 레버도 추가해야함.

4-5 동료를 구출하기 위해 문 앞에서 F키를 이용하여 상호작용하게 되면 타이밍체크 팝업창이 뜨게되며 Space바를 이용해 일정한 칸에 맞추게 되면 타이밍체크 팝업창이 꺼지고 비밀번호 팝업창이 뜬다.(3-9 임형균) 50%
    동료배치, 문 배치, F상호작용버튼 구현시 타이밍체크 팝업창 뜨게 추가해야함.

### 5주차

5-1 ~~비밀번호 팝업창은 갇혀있는 동물의 갇혀있는 동과 호수를 입력한 후 확인 버튼을 누르면 팝업창이 닫히게되고 동료 동물 리스트에 추가된다.~~ (4-2 임형균) (100%)

5-2 ~~철퇴 함정은 맵 중간중간 등장하며 진자운동을 하며 길을 막고 있으며 플레이어와 충돌시 사망하며 Life가 1 감소한다.~~ (2-2 이승현, 임형균) (100%)
...
## 3. 키보드 이벤트에 대한 흐름도 flowchart <a name='8'></a>
...
4-4 1스테이지 컨셉인 실험실 레벨디자인을 에셋을 활용하여 구현한다. (4-2 이승현) (50%)
...
5-1 비밀번호 팝업창은 갇혀있는 동물의 영어 이름을 입력한 후 확인 버튼을 누르면 팝업창이 닫히게되고 동료 동물 리스트에 추가된다.(4-2 임형균) (90%)
